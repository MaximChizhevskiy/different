const personalPlanPeter = {
  ...
  skills: {
    ...
    showAgeAndLangs: function(ageAndLangs){  // Решение 3й задачи;
    const {age} = ageAndLangs;
    const {languages} = ageAndLangs.skills;
    let str = `Мне ${age} и я владею языками: `;
    languages.forEach(function(langs){
    str += `${langs.toUpperCase()} `;
    });
    return str;
    } 
...
function showProgrammingLangs (progLangs) {
    const {programmingLangs} = progLangs.skills;
    let str = ''; 
